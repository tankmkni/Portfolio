...
//	マークの動きを決める変数( true = アナログ風、false = デジタル風 )
var analogMotion = false;
...
function drawDocoTsubuClock(){
	...
	//imgのセット
//	canvas.style.backgroundImage = 'url(img/docoTsubu_pink.PNG)';
...
function roundMark( img ){
	...
	if( analogMotion ){
		if( sec == tmpSec ){
			if( secTenth == 5 ){//値が「10」だとカクカクな動き（処理速度？）
				degTenth += 0.6;
				secTenth = 0;
				degHundredth = 0;
			}else{
